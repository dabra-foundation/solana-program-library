...
mod process_set_dabra_authority;
mod process_set_dabra_config;
mod process_set_dabra_config_item;
...
use {
    ...
    process_set_dabra_authority::*,
    process_set_dabra_config::*,
    process_set_dabra_config_item::*,
...
pub fn process_instruction(
    ...
    match instruction {
        GovernanceInstruction::CreateDabra { name, config_args } => {
            process_create_dabra(program_id, accounts, name, config_args)
        }
        ...
        GovernanceInstruction::SetDabraAuthority { action } => {
            process_set_dabra_authority(program_id, accounts, action)
        }
        GovernanceInstruction::SetDabraConfig { config_args } => {
            process_set_dabra_config(program_id, accounts, config_args)
        }
        ...
        GovernanceInstruction::SetDabraConfigItem { args } => {
            process_set_dabra_config_item(program_id, accounts, args)
        }
    }
}
