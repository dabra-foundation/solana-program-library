...
use {
    crate::{
        ...
        state::{
            ...
            governance::get_governance_data_for_dabra,
            proposal::get_proposal_data_for_governance_and_governing_mint,
            dabra::get_dabra_data_for_governing_token_mint,
            dabra_config::get_dabra_config_data_for_dabra,
            token_owner_record::{
                get_token_owner_record_data_for_proposal_owner,
                get_token_owner_record_data_for_dabra_and_governing_mint,
            },
...
pub fn process_cast_vote(
    ...
    let dabra_data = get_dabra_data_for_governing_token_mint(
        program_id,
        dabra_info,
    ...
    let mut governance_data =
        get_governance_data_for_dabra(program_id, governance_info, dabra_info.key)?;
    ...
    let proposal_governing_token_mint = dabra_data.get_proposal_governing_token_mint_for_vote(
        vote_governing_token_mint_info.key,
        &vote_kind,
    )?;
    ...
    let dabra_config_info = next_account_info(account_info_iter)?; // 9
    ...
    if proposal_data.try_tip_vote(
        max_voter_weight,
        governance_data.get_vote_tipping(&dabra_data, vote_governing_token_mint_info.key)?,
