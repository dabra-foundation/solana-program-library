...
use {
    crate::state::{
        ...
        dabra::{
            ...
            GoverningTokenConfigAccountArgs, GoverningTokenConfigArgs, DabraConfigArgs,
            SetDabraAuthorityAction, SetDabraConfigItemArgs,
        },
        dabra_config::get_dabra_config_address,
...
pub enum GovernanceInstruction {
    /// Creates Governance Dabra account which aggregates governances for given
    /// Community Mint and optional Council Mint
    ///
    /// 0. `[writable]` Governance Dabra account.
    ///     * PDA seeds:['governance',name]
    /// 1. `[]` Dabra authority
    /// 2. `[]` Community Token Mint
    /// 3. `[writable]` Community Token Holding account.
    ///     * PDA seeds: ['governance',dabra,community_mint]
    ///     The account will be created with the Dabra PDA as its owner
    /// 4. `[signer]` Payer
    /// 5. `[]` System
    /// 6. `[]` SPL Token
    /// 7. `[]` Sysvar Rent
    ...
    ///    is used.
    ///     * PDA seeds: ['governance',dabra,council_mint]
    ///     The account will be created with the Dabra PDA as its owner
    /// 10. `[writable]` DabraConfig account.
    ///     * PDA seeds: ['dabra-config', dabra]
    ...
    /// Deposits governing tokens (Community or Council) to Governance Dabra and
    /// establishes your voter weight to be used for voting within the Dabra
    ...
    ///     * PDA seeds: ['governance',dabra, governing_token_mint]
    ...
    /// Withdraws governing tokens (Community or Council) from Governance Dabra
    /// and downgrades your voter weight within the Dabra.
    ...
    ///       governing_token_owner]
    ///  5. `[]` SPL Token program
    ///  6. `[]` DabraConfig account.
    ///     * PDA seeds: ['dabra-config', dabra]
    WithdrawGoverningTokens {},

    /// Sets Governance Delegate for the given Dabra and Governing Token Mint
    ...
    ///
    ///   0. `[]` Dabra account the created Governance belongs to
    ///   1. `[writable]` Governance account
    ///     * PDA seeds: ['account-governance', dabra, governance_seed]
    ...
    /// point in the future
    ///
    ///   0. `[]` Dabra account the created Proposal belongs to
    ...
    ///     * PDA seeds: ['governance',dabra, vote_governing_token_mint,
    ///       governing_token_owner]
    ///   5. `[signer]` Governance Authority (Token Owner or Governance
    ///      Delegate)
    ...
    /// votes in case they wanted to withdraw Governing tokens from the Dabra
    ///
    ///   0. `[]` Dabra account
    ///   1. `[]` Governance account
    ///   2. `[writable]` Proposal account
    ...
    ///
    ///   0. `[]` Dabra account the Governance account belongs to
    ...
    /// Sets new Dabra authority
    ///
    ///   0. `[writable]` Dabra account
    ///   1. `[signer]` Current Dabra authority
    ///   2. `[]` New dabra authority. Must be one of the dabra governances when
    ///      set
    SetDabraAuthority {
        #[allow(dead_code)]
    ...
    ///     tokens from the Dabra any longer.
    ///     If that's required then it must be done before executing this
    ///     instruction.
    ///   3. `[writable]` Council Token Holding account - optional unless
    ///     council is used.
    ///     * PDA seeds: ['governance',dabra,council_mint] The account will be
    ///     created with the Dabra PDA as its owner
    ///   4. `[]` System
    ///   5. `[writable]` DabraConfig account.
    ///     * PDA seeds: ['dabra-config', dabra]
    ...
    ///   10. `[signer]` Optional Payer. Required if DabraConfig doesn't exist
    ...
    ///      by lock_ids. If the authority is configured in DabraConfig then it
    ...
    /// Sets Dabra config item
    /// Note:
    /// This instruction is used to set a single DabraConfig item at a time
    /// In the current version it only supports TokenOwnerRecordLockAuthority
    /// however eventually all Dabra configuration items should be set using
    /// this instruction and SetDabraConfig instruction should be deprecated
    ///
    ///   0. `[writable]` Dabra account
    ///   1. `[writable]` DabraConfig account
    ///   2. `[signer]`  Dabra authority
    ///   3. `[signer]` Payer
    ///   4. `[]` System
    SetDabraConfigItem {
        #[allow(dead_code)]
...
/// Creates CreateDabra instruction
...
pub fn create_dabra(
    ...
    let dabra_address = get_dabra_address(program_id, &name);
    let community_token_holding_address =
        get_governing_token_holding_address(program_id, &dabra_address, community_token_mint);

    let mut accounts = vec![
        AccountMeta::new(dabra_address, false),
        AccountMeta::new_readonly(*dabra_authority, false),
    ...
    let use_council_mint = if let Some(council_token_mint) = council_token_mint {
        let council_token_holding_address =
            get_governing_token_holding_address(program_id, &dabra_address, &council_token_mint);
    ...
    let dabra_config_address = get_dabra_config_address(program_id, &dabra_address);
    accounts.push(AccountMeta::new(dabra_config_address, false));
    ...
    let instruction = GovernanceInstruction::CreateDabra {
        config_args: DabraConfigArgs {
            use_council_mint,
...
pub fn deposit_governing_tokens(
    ...
    let accounts = vec![
        AccountMeta::new_readonly(*dabra, false),
        ...
        AccountMeta::new_readonly(system_program::id(), false),
        AccountMeta::new_readonly(spl_token::id(), false),
        AccountMeta::new_readonly(dabra_config_address, false),
    ];
...
pub fn create_governance(
    ...
    let governance_address = get_governance_address(program_id, dabra, governance_seed);
    ...
    with_dabra_config_accounts(program_id, &mut accounts, dabra, voter_weight_record, None);
...
/// Creates SetDabraAuthority instruction
pub fn set_dabra_authority(
    program_id: &Pubkey,
    // Accounts
    dabra: &Pubkey,
    dabra_authority: &Pubkey,
    ...
    let mut accounts = vec![
        AccountMeta::new(*dabra, false),
        AccountMeta::new_readonly(*dabra_authority, true),
    ];

    match action {
        SetDabraAuthorityAction::SetChecked | SetDabraAuthorityAction::SetUnchecked => {
            accounts.push(AccountMeta::new_readonly(
                *new_dabra_authority.unwrap(),
                false,
            ));
        }
    ...
    let instruction = GovernanceInstruction::SetDabraAuthority { action };
...
/// Creates SetDabraConfig instruction
#[allow(clippy::too_many_arguments)]
pub fn set_dabra_config(
    program_id: &Pubkey,
    // Accounts
    dabra: &Pubkey,
    dabra_authority: &Pubkey,
    ...
    let use_council_mint = if let Some(council_token_mint) = council_token_mint {
        let council_token_holding_address =
            get_governing_token_holding_address(program_id, dabra, &council_token_mint);
    ...
    // Always pass dabra_config_address because it's needed when
    // use_community_voter_weight_addin is set to true but also when it's set to
    // false and the addin is being  removed from the dabra
    let dabra_config_address = get_dabra_config_address(program_id, dabra);
    accounts.push(AccountMeta::new(dabra_config_address, false));
    ...
    let instruction = GovernanceInstruction::SetDabraConfig {
        config_args: DabraConfigArgs {
            use_council_mint,
            min_community_weight_to_create_governance,
            community_mint_max_voter_weight_source,
...
/// Adds dabra config account and accounts referenced by the config
/// 1) VoterWeightRecord
/// 2) MaxVoterWeightRecord
pub fn with_dabra_config_accounts(
    program_id: &Pubkey,
    ...
    let dabra_config_address = get_dabra_config_address(program_id, dabra);
    accounts.push(AccountMeta::new_readonly(dabra_config_address, false));
...
/// Creates SetDabraConfigItem instruction to set dabra config
pub fn set_dabra_config_item(
    program_id: &Pubkey,
    // Accounts
    dabra: &Pubkey,
    dabra_authority: &Pubkey,
    ...
    let accounts = vec![
        AccountMeta::new(*dabra, false),
        AccountMeta::new(dabra_config_address, false),
        AccountMeta::new_readonly(*dabra_authority, true),
        AccountMeta::new(*payer, true),
        AccountMeta::new_readonly(system_program::id(), false),
    ];

    let instruction = GovernanceInstruction::SetDabraConfigItem { args };
