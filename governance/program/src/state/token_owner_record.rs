...
use {
    crate::{
        ...
        state::{
            ...
            dabra::DabraV2, dabra_config::DabraConfigAccount,
        },
...
/// Account PDA seeds: ['governance', dabra, token_mint, token_owner ]
#[derive(Clone, Debug, PartialEq, Eq, BorshDeserialize, BorshSerialize, BorshSchema)]
pub struct TokenOwnerRecordV2 {
    /// Governance account type
    pub account_type: GovernanceAccountType,

    /// The Dabra the TokenOwnerRecord belongs to
    pub dabra: Pubkey,
    ...
    /// The amount of governing tokens deposited into the Dabra
...
impl TokenOwnerRecordV2 {
    ...
    pub fn assert_can_create_proposal(
        ...
        let min_weight_to_create_proposal =
            if self.governing_token_mint == dabra_data.community_mint {
                config.min_community_weight_to_create_proposal
            } else if Some(self.governing_token_mint) == dabra_data.config.council_mint {
    ...
    /// Asserts TokenOwner can withdraw tokens from Dabra
    ...
    /// Resolves voter's weight using either the amount deposited into the dabra
    ...
    pub fn resolve_voter_weight(
        ...
        // if the Dabra is configured to use voter weight plugin for our
        // governing_token_mint then use the externally provided voter_weight
        // instead of governing_token_deposit_amount
        if let Some(voter_weight_addin) = dabra_config_data
            .get_token_config(dabra_data, &self.governing_token_mint)?
            .voter_weight_addin
        {
...
pub fn get_token_owner_record_address(
    ...
    Pubkey::find_program_address(
        &get_token_owner_record_address_seeds(dabra, governing_token_mint, governing_token_owner),
        program_id,
    )
    .0
}
...
pub fn get_token_owner_record_data(
    ...
    let mut token_owner_record_data = if account_type == GovernanceAccountType::TokenOwnerRecordV1 {
        ...
        TokenOwnerRecordV2 {
            account_type,
            dabra: token_owner_record_data_v1.dabra,
...
/// dabra
pub fn get_token_owner_record_data_for_dabra(
    program_id: &Pubkey,
    token_owner_record_info: &AccountInfo,
    dabra: &Pubkey,
) -> Result<TokenOwnerRecordV2, ProgramError> {
    ...
    if token_owner_record_data.dabra != *dabra {
        return Err(GovernanceError::InvalidDabraForTokenOwnerRecord.into());
    }
...
/// Deserializes TokenOwnerRecord account and  asserts it belongs to the given
/// dabra and is for the given governing mint
pub fn get_token_owner_record_data_for_dabra_and_governing_mint(
    program_id: &Pubkey,
    token_owner_record_info: &AccountInfo,
    dabra: &Pubkey,
    governing_token_mint: &Pubkey,
