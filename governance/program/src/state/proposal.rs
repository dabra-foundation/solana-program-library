...
use {
    crate::{
        addins::max_voter_weight::{
            ...
            get_max_voter_weight_record_data_for_dabra_and_governing_token_mint,
        },
        ...
        state::{
            ...
            dabra::DabraV2,
            dabra_config::DabraConfigAccount,
...
impl ProposalV2 {
    ...
    /// Calculates max voter weight for given mint supply and dabra config
    fn get_max_voter_weight_from_mint_supply(
        ...
        if Some(*governing_token_mint) == dabra_data.config.council_mint {
            return Ok(governing_token_mint_supply);
        }

        let max_voter_weight = match dabra_data.config.community_mint_max_voter_weight_source {
    ...
    pub fn resolve_max_voter_weight(
        ...
        dabra: &Pubkey,
        dabra_data: &DabraV2,
        dabra_config_data: &DabraConfigAccount,
        vote_governing_token_mint_info: &AccountInfo,
        vote_kind: &VoteKind,
    ) -> Result<u64, ProgramError> {
        // if the Dabra is configured to use max voter weight for the given voting
        // governing_token_mint then use the externally provided max_voter_weight
        // instead of the supply based max
        if let Some(max_voter_weight_addin) = dabra_config_data
            .get_token_config(dabra_data, vote_governing_token_mint_info.key)?
...
mod test {
    ...
    proptest! {
        ...
        fn test_try_tip_vote(test_case in vote_casting_test_cases()) {
           ...
           proposal.deny_vote_weight = Some(test_case.no_votes_count);
            ...
            let max_voter_weight = proposal.get_max_voter_weight_from_mint_supply(&dabra,&governing_token_mint, test_case.governing_token_supply,&vote_kind).unwrap();
    ...
    proptest! {
        ...
        fn test_try_tip_vote_with_full_vote_results(
            ...
            let max_voter_weight = proposal.get_max_voter_weight_from_mint_supply(&dabra,&governing_token_mint,governing_token_supply,&vote_kind).unwrap();
    ...
    fn test_try_tip_vote_with_reduced_community_mint_max_vote_weight() {
        ...
        let mut dabra = create_test_dabra();
        let governing_token_mint = proposal.governing_token_mint;
        let vote_kind = VoteKind::Electorate;
        let vote_tipping = VoteTipping::Strict;

        // reduce max vote weight to 100
        dabra.config.community_mint_max_voter_weight_source =
    ...
    fn test_try_tip_vote_for_council_vote_with_reduced_community_mint_max_vote_weight() {
        ...
        dabra.config.council_mint = Some(proposal.governing_token_mint);
    ...
    pub fn test_finalize_vote_with_expired_voting_time_error() {
        ...
        let max_voter_weight = proposal
            .get_max_voter_weight_from_mint_supply(&dabra, &governing_token_mint, 100, &vote_kind)
            .unwrap();
