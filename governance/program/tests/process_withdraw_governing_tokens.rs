...
use {
    ...
    spl_governance::{
        ...
        state::{
            dabra_config::GoverningTokenType, token_owner_record::get_token_owner_record_address,
        },
    },
};
...
async fn test_withdraw_community_tokens() {
    ...
    // Act
    governance_test
        .withdraw_community_tokens(&dabra_cookie, &token_owner_record_cookie)
        .await
        .unwrap();
...
async fn test_withdraw_council_tokens() {
    ...
    // Act
    governance_test
        .withdraw_council_tokens(&dabra_cookie, &token_owner_record_cookie)
        .await
        .unwrap();
...
async fn test_withdraw_tokens_with_malicious_holding_account_error() {
    ...
    // Try to maliciously withdraw from other token account owned by dabra

    let dabra_token_account_cookie = governance_test
        .bench
        .with_token_account(
            &dabra_cookie.account.community_mint,
            &dabra_cookie.address,
    ...
    withdraw_ix.accounts[1].pubkey = dabra_token_account_cookie.address;
...
async fn test_withdraw_dormant_community_tokens() {
    ...
    dabra_setup_args.community_token_config_args.token_type = GoverningTokenType::Dormant;
