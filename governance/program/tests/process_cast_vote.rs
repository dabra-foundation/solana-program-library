...
#[tokio::test]
async fn test_cast_vote_with_invalid_token_owner_record_from_different_dabra_error() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    ...
    // Try to use token_owner_record from another Dabra for the same mint
    let dabra_cookie2 = governance_test.with_dabra_using_mints(&dabra_cookie).await;
    ...
    assert_eq!(err, GovernanceError::InvalidDabraForTokenOwnerRecord.into());
}
...
async fn test_cast_vote_with_strict_vote_tipped_to_succeeded() {
    ...
    let mut governance_cookie = governance_test
        .with_governance(&dabra_cookie, &token_owner_record_cookie1)
        .await
        .unwrap();
    ...
    governance_test
        .mint_community_tokens(&dabra_cookie, 20)
        .await;
...
async fn test_cast_vote_with_early_vote_tipped_to_succeeded() {
    ...
    governance_test
        .mint_community_tokens(&dabra_cookie, 500) // total supply: 1000
        .await;
...
async fn test_cast_vote_with_threshold_below_50_and_vote_not_tipped() {
    ...
    // Total 210 tokens
    governance_test
        .mint_community_tokens(&dabra_cookie, 110)
        .await;
...
async fn test_cast_vote_with_disabled_tipping_yes_votes() {
    ...
    governance_test
        .mint_community_tokens(&dabra_cookie, 20) // total supply: 120
        .await;
    let proposal_cookie = governance_test
        .with_signed_off_proposal(&token_owner_record_cookie1, &mut _governance_cookie)
        .await
...
async fn test_cast_vote_with_cast_twice_error() {
    ...
    governance_test
        .mint_community_tokens(&dabra_cookie, 200)
        .await;
...
#[tokio::test]
async fn test_cast_vote_with_invalid_dabra_config_account_address_error() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    ...
    let err = governance_test
        .with_cast_vote_using_instruction(
            ...
            Vote::Deny,
            |i| {
                i.accounts[10].pubkey = dabra_config_address; // dabra_config_address
            },
            None,
        )
        .await
