...
async fn test_create_multiple_proposals() {
    ...
    let mut governance_cookie = governance_test
        .with_governance(&dabra_cookie, &community_token_owner_record_cookie)
        .await
        .unwrap();
...
async fn test_create_proposal_with_governance_delegate_signer() {
    ...
    governance_test
        .with_community_governance_delegate(&dabra_cookie, &mut token_owner_record_cookie)
        .await;
...
async fn test_create_proposal_with_invalid_governing_token_mint_error() {
    ...
    // Try to use mint which  doesn't belong to the Dabra
...
async fn test_create_council_proposal_using_community_tokens() {
    ...
    let mut governance_cookie = governance_test
        .with_governance(&dabra_cookie, &council_token_owner_record_cookie)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_create_proposal_with_invalid_dabra_config_account_address_error() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    ...
    let err = governance_test
        .with_proposal_using_instruction(&token_owner_record_cookie, &mut governance_cookie, |i| {
            i.accounts[8].pubkey = dabra_config_address;
        })
        .await
        .err()
        .unwrap();
