...
use {
    ...
    spl_governance::{
        ...
        state::{dabra::get_governing_token_holding_address, dabra_config::GoverningTokenType},
    },
...
async fn test_revoke_community_tokens() {
    ...
    let mut dabra_config_args = DabraSetupArgs::default();
    dabra_config_args.community_token_config_args.token_type = GoverningTokenType::Membership;
    ...
    // Act
    governance_test
        .revoke_community_tokens(&dabra_cookie, &token_owner_record_cookie)
        .await
        .unwrap();
    ...
    let holding_account = governance_test
        .get_token_account(&dabra_cookie.community_token_holding_account)
        .await;
...
async fn test_revoke_council_tokens() {
    ...
    let mut dabra_config_args = DabraSetupArgs::default();
    dabra_config_args.council_token_config_args.token_type = GoverningTokenType::Membership;
    ...
    let token_owner_record_cookie = governance_test
        .with_council_token_deposit(&dabra_cookie)
        .await
        .unwrap();

    // Act
    governance_test
        .revoke_council_tokens(&dabra_cookie, &token_owner_record_cookie)
        .await
        .unwrap();
    ...
    let holding_account = governance_test
        .get_token_account(&dabra_cookie.council_token_holding_account.unwrap())
        .await;
...
async fn test_revoke_community_tokens_with_cannot_revoke_dormant_token_error() {
    ...
    let mut dabra_config_args = DabraSetupArgs::default();
    dabra_config_args.community_token_config_args.token_type = GoverningTokenType::Dormant;

    governance_test
        .set_dabra_config(&mut dabra_cookie, &dabra_config_args)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_revoke_council_tokens_with_other_dabra_config_account_error() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    ...
    let err = governance_test
        .revoke_governing_tokens_using_instruction(
            ...
            dabra_cookie.council_mint_authority.as_ref().unwrap(),
            1,
            |i| i.accounts[5].pubkey = dabra_cookie2.dabra_config.address, //dabra_config_address
            None,
        )
        .await
        .err()
...
#[tokio::test]
async fn test_revoke_council_tokens_with_invalid_dabra_config_account_address_error() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    ...
    let dabra_config_address = Pubkey::new_unique();
    ...
    let err = governance_test
        .revoke_governing_tokens_using_instruction(
            ...
            dabra_cookie.council_mint_authority.as_ref().unwrap(),
            1,
            |i| i.accounts[5].pubkey = dabra_config_address, // dabra_config_address
            None,
        )
        .await
        .err()
    ...
    assert_eq!(err, GovernanceError::InvalidDabraConfigAddress.into());
}
...
async fn test_revoke_council_tokens_with_community_mint_error() {
    ...
    let governing_token_mint = dabra_cookie.account.community_mint;
    let governing_token_mint_authority = clone_keypair(&dabra_cookie.community_mint_authority);
    let governing_token_holding_address = get_governing_token_holding_address(
        &governance_test.program_id,
        &dabra_cookie.address,
