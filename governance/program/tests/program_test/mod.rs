...
use {
    ...
    spl_governance::{
        instruction::{
            ...
            create_governance, create_native_treasury, create_proposal, create_dabra,
            ...
            set_governance_delegate, set_dabra_authority, set_dabra_config, set_dabra_config_item,
        ...
        state::{
            ...
            dabra::{
                get_governing_token_holding_address, get_dabra_address,
                GoverningTokenConfigAccountArgs, DabraConfig, DabraV2, SetDabraAuthorityAction,
                SetDabraConfigItemArgs,
            },
            dabra_config::{get_dabra_config_address, GoverningTokenConfig, DabraConfigAccount},
...
use {
    crate::{
        args::{PluginSetupArgs, DabraSetupArgs},
        cookies::{
            ...
            ProposalTransactionCookie, DabraCookie, TokenOwnerRecordCookie,
            TokenOwnerRecordLockCookie, VoteRecordCookie,
        },
        program_test::cookies::{
            DabraConfigCookie, SignatoryRecordCookie, VoterWeightRecordCookie,
        },
    },
...
impl GovernanceProgramTest {
    ...
    #[allow(dead_code)]
    pub async fn with_dabra(&mut self) -> DabraCookie {
        let dabra_setup_args = DabraSetupArgs::default();
        self.with_dabra_using_args(&dabra_setup_args).await
    }

    #[allow(dead_code)]
    pub async fn with_dabra_using_addins(
        &mut self,
    ...
    #[allow(dead_code)]
    pub async fn with_dabra_using_args(
        &mut self,
        dabra_setup_args: &DabraSetupArgs,
    ) -> DabraCookie {
        let name = format!("Dabra #{}", self.next_dabra_id).to_string();
        self.next_dabra_id += 1;

        let dabra_address = get_dabra_address(&self.program_id, &name);
        ...
        ) = if dabra_setup_args.use_council_mint {
        ...
        let community_token_args = GoverningTokenConfigAccountArgs {
            voter_weight_addin: dabra_setup_args
                .community_token_config_args
                .voter_weight_addin,
            max_voter_weight_addin: dabra_setup_args
                .community_token_config_args
                .max_voter_weight_addin,
            token_type: dabra_setup_args
                .community_token_config_args
                .token_type
                .clone(),
        };
        ...
        let create_dabra_ix = create_dabra(
            &self.program_id,
            ...
            name.clone(),
            dabra_setup_args.min_community_weight_to_create_governance,
            dabra_setup_args
        ...
        self.bench
            .process_transaction(&[create_dabra_ix], None)
            .await
            .unwrap();
        ...
        let account = DabraV2 {
            ...
            name,
            reserved: [0; 6],
            authority: Some(dabra_authority.pubkey()),
            config: DabraConfig {
                council_mint: council_token_mint_pubkey,
                reserved: [0; 6],

                min_community_weight_to_create_governance: dabra_setup_args
                    .min_community_weight_to_create_governance,
                community_mint_max_voter_weight_source: dabra_setup_args
                    .community_mint_max_voter_weight_source
                    .clone(),
                legacy1: 0,
                legacy2: 0,
        ...
        let dabra_config_cookie = DabraConfigCookie {
            address: get_dabra_config_address(&self.program_id, &dabra_address),
            account: DabraConfigAccount {
        ...
        DabraCookie {
            ...
            dabra_authority: Some(dabra_authority),
            dabra_config: dabra_config_cookie,
        }
    }

    #[allow(dead_code)]
    pub async fn with_dabra_using_mints(&mut self, dabra_cookie: &DabraCookie) -> DabraCookie {
        let name = format!("Dabra #{}", self.next_dabra_id).to_string();
        self.next_dabra_id += 1;

        let dabra_address = get_dabra_address(&self.program_id, &name);
        let council_mint = dabra_cookie.account.config.council_mint.unwrap();
        ...
        let create_dabra_ix = create_dabra(
            &self.program_id,
            &dabra_authority.pubkey(),
            &dabra_cookie.account.community_mint,
        ...
        let account = DabraV2 {
            account_type: GovernanceAccountType::DabraV2,
            community_mint: dabra_cookie.account.community_mint,
        ...
        let council_token_holding_address =
            get_governing_token_holding_address(&self.program_id, &dabra_address, &council_mint);
        ...
        DabraCookie {
            ...
            community_mint_authority: clone_keypair(&dabra_cookie.community_mint_authority),
            community_token_holding_account: community_token_holding_address,

            council_token_holding_account: Some(council_token_holding_address),
            council_mint_authority: Some(clone_keypair(
                dabra_cookie.council_mint_authority.as_ref().unwrap(),
            )),
            dabra_authority: Some(dabra_authority),
            dabra_config: dabra_config_cookie,
        }
    ...
    pub async fn with_community_token_owner_record(
        ...
        self.with_token_owner_record(dabra_cookie, &dabra_cookie.account.community_mint)
            .await
    }
    ...
    pub async fn with_council_token_owner_record(
        ...
        self.with_token_owner_record(
            dabra_cookie,
            &dabra_cookie.account.config.council_mint.unwrap(),
        )
        .await
    }
    ...
    #[allow(dead_code)]
    pub async fn mint_community_tokens(&mut self, dabra_cookie: &DabraCookie, amount: u64) {
    ...
    #[allow(dead_code)]
    pub async fn mint_council_tokens(&mut self, dabra_cookie: &DabraCookie, amount: u64) {
        let token_account_keypair = Keypair::new();
        let council_mint = dabra_cookie.account.config.council_mint.unwrap();
    ...
    #[allow(dead_code)]
    pub async fn set_dabra_authority(
        &mut self,
        dabra_cookie: &DabraCookie,
        new_dabra_authority: Option<&Pubkey>,
    ) -> Result<(), ProgramError> {
        self.set_dabra_authority_using_instruction(
            dabra_cookie,
            new_dabra_authority,
            true,
            NopOverride,
    ...
    #[allow(dead_code)]
    pub async fn set_dabra_authority_impl(
        &mut self,
        dabra_cookie: &DabraCookie,
        new_dabra_authority: Option<&Pubkey>,
    ...
    #[allow(dead_code)]
    pub async fn set_dabra_authority_using_instruction<F: Fn(&mut Instruction)>(
        &mut self,
        dabra_cookie: &DabraCookie,
        new_dabra_authority: Option<&Pubkey>,
        check_authority: bool,
        ...
        let action = if new_dabra_authority.is_some() {
            if check_authority {
                SetDabraAuthorityAction::SetChecked
            } else {
                SetDabraAuthorityAction::SetUnchecked
            }
        } else {
            SetDabraAuthorityAction::Remove
        };

        let mut set_dabra_authority_ix = set_dabra_authority(
            &self.program_id,
            &dabra_cookie.address,
            &dabra_cookie.dabra_authority.as_ref().unwrap().pubkey(),
            new_dabra_authority,
            action,
        );

        instruction_override(&mut set_dabra_authority_ix);

        let default_signers = &[dabra_cookie.dabra_authority.as_ref().unwrap()];
        let signers = signers_override.unwrap_or(default_signers);

        self.bench
            .process_transaction(&[set_dabra_authority_ix], Some(signers))
            .await
    }

    #[allow(dead_code)]
    pub async fn set_dabra_config(
        &mut self,
        dabra_cookie: &mut DabraCookie,
        dabra_setup_args: &DabraSetupArgs,
    ) -> Result<(), ProgramError> {
        self.set_dabra_config_using_instruction(dabra_cookie, dabra_setup_args, NopOverride, None)
            .await
    }

    #[allow(dead_code)]
    pub async fn set_dabra_config_using_instruction<F: Fn(&mut Instruction)>(
        &mut self,
        dabra_cookie: &mut DabraCookie,
        dabra_setup_args: &DabraSetupArgs,
        instruction_override: F,
        ...
        let council_token_mint = if dabra_setup_args.use_council_mint {
            dabra_cookie.account.config.council_mint
        } else {
            None
        };

        let mut set_dabra_config_ix = set_dabra_config(
            &self.program_id,
            &dabra_cookie.address,
            &dabra_cookie.dabra_authority.as_ref().unwrap().pubkey(),
            council_token_mint,
            &self.bench.payer.pubkey(),
            Some(dabra_setup_args.community_token_config_args.clone()),
            Some(dabra_setup_args.council_token_config_args.clone()),
            dabra_setup_args.min_community_weight_to_create_governance,
            dabra_setup_args
                .community_mint_max_voter_weight_source
                .clone(),
        ...
        instruction_override(&mut set_dabra_config_ix);

        let default_signers = &[dabra_cookie.dabra_authority.as_ref().unwrap()];
        let signers = signers_override.unwrap_or(default_signers);

        dabra_cookie.account.config.council_mint = council_token_mint;
        dabra_cookie
            .account
            .config
            .community_mint_max_voter_weight_source = dabra_setup_args
        ...
        dabra_cookie.dabra_config = DabraConfigCookie {
            address: get_dabra_config_address(&self.program_id, &dabra_cookie.address),
            account: DabraConfigAccount {
                account_type: GovernanceAccountType::DabraConfig,
                dabra: dabra_cookie.address,
                reserved: Reserved110::default(),
        ...
        self.bench
            .process_transaction(&[set_dabra_config_ix], Some(signers))
            .await
    }
    ...
    pub async fn with_governance(
        ...
        self.with_governance_using_config(dabra_cookie, token_owner_record_cookie, &config)
            .await
    }
    ...
    pub async fn with_governance_impl(
        ...
        let governance_address =
            get_governance_address(&self.program_id, &dabra_cookie.address, &governance_seed);
    ...
    pub async fn with_proposal_using_instruction_impl<F: Fn(&mut Instruction)>(
        ...
        let mut create_proposal_transaction = create_proposal(
            ...
            &self.bench.payer.pubkey(),
            voter_weight_record,
            &governance_cookie.account.dabra,
            name.clone(),
        ...
        Ok(ProposalCookie {
            ...
            dabra: governance_cookie.account.dabra,
    ...
    pub async fn relinquish_vote_using_instruction<F: Fn(&mut Instruction)>(
        ...
        let mut relinquish_vote_ix = relinquish_vote(
            &self.program_id,
            &token_owner_record_cookie.account.dabra,
            &proposal_cookie.account.governance,
            &proposal_cookie.address,
            &token_owner_record_cookie.address,
            &token_owner_record_cookie.account.governing_token_mint,
    ...
    pub async fn with_governance_with_required_signatory(
        ...
        let dabra_cookie = self.with_dabra().await;
    ...
    #[allow(dead_code)]
    pub async fn get_dabra_account(&mut self, dabra_address: &Pubkey) -> DabraV2 {
        self.bench.get_borsh_account::<DabraV2>(dabra_address).await
    }

    #[allow(dead_code)]
    pub async fn get_dabra_config_account(
        &mut self,
        dabra_config_address: &Pubkey,
    ) -> DabraConfigAccount {
        self.bench
            .get_borsh_account::<DabraConfigAccount>(dabra_config_address)
            .await
    }

    #[allow(dead_code)]
    pub fn remove_dabra_config_account(&mut self, dabra_config_address: &Pubkey) {
        self.bench.remove_account(dabra_config_address);
    }
    ...
    pub async fn with_voter_weight_addin_record_impl(
        ...
        let voter_weight_record_cookie = VoterWeightRecordCookie {
            ...
            account: VoterWeightRecord {
                ...
                dabra: token_owner_record_cookie.account.dabra,
                governing_token_mint: token_owner_record_cookie.account.governing_token_mint,
    ...
    #[allow(dead_code)]
    pub async fn set_dabra_config_item(
        &mut self,
        dabra_cookie: &DabraCookie,
        args: SetDabraConfigItemArgs,
    ) -> Result<(), ProgramError> {
        self.set_dabra_config_item_using_ix(dabra_cookie, args, NopOverride, None)
            .await
    }

    #[allow(dead_code)]
    pub async fn set_dabra_config_item_using_ix<F: Fn(&mut Instruction)>(
        &mut self,
        dabra_cookie: &DabraCookie,
        args: SetDabraConfigItemArgs,
        instruction_override: F,
        ...
        let mut set_dabra_config_item_ix = set_dabra_config_item(
            &self.program_id,
            &dabra_cookie.address,
            &dabra_cookie.account.authority.unwrap(),
            &self.bench.payer.pubkey(),
            args,
        );

        instruction_override(&mut set_dabra_config_item_ix);

        let default_signers = &[dabra_cookie.dabra_authority.as_ref().unwrap()];
        let signers = signers_override.unwrap_or(default_signers);

        self.bench
            .process_transaction(&[set_dabra_config_item_ix], Some(signers))
            .await
    }
    ...
    pub async fn with_community_token_owner_record_lock_authority(
        ...
        self.set_dabra_config_item(dabra_cookie, args)
            .await
            .unwrap();
