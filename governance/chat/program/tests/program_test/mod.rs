use {
    ...
    spl_governance::{
        instruction::{
            create_governance, create_proposal, create_dabra, create_token_owner_record,
        ...
        state::{
            ...
            dabra::{get_dabra_address, GoverningTokenConfigAccountArgs},
            dabra_config::GoverningTokenType,
...
impl GovernanceChatProgramTest {
    ...
    pub async fn with_proposal(&mut self) -> ProposalCookie {
        // Create Dabra
        let name = self.bench.get_unique_name("dabra");

        let dabra_address = get_dabra_address(&self.governance_program_id, &name);
