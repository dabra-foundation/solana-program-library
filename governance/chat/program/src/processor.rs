...
use {
    ...
    spl_governance::state::{
        governance::get_governance_data_for_dabra, proposal::get_proposal_data_for_governance,
...
pub fn process_post_message(
    ...
    let dabra_info = next_account_info(account_info_iter)?; // 1
    ...
    let dabra_data = get_dabra_data(governance_program_id, dabra_info)?;

    let governance_data =
        get_governance_data_for_dabra(governance_program_id, governance_info, dabra_info.key)?;

    let token_owner_record_data = get_token_owner_record_data_for_dabra(
        governance_program_id,
        token_owner_record_info,
        &governance_data.dabra,
    )?;

    token_owner_record_data.assert_token_owner_or_delegate_is_signer(governance_authority_info)?;

    // deserialize proposal to assert it belongs to the given governance and hence
    // belongs to the same dabra as the token owner
    ...
    let dabra_config_data =
        get_dabra_config_data_for_dabra(governance_program_id, dabra_config_info, dabra_info.key)?;
    ...
    // different level is required then it should be added to dabra config
    if voter_weight < 1 {
